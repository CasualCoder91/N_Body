...
def select2dPositions(conn, simulationID):
    cur = conn.cursor()
    cur.execute("""SELECT position.timestep,position2D.x,position2D.y FROM star
    ...
    with conn:
        ...
        for i in np.unique(data[:,0]):
            ...
            if(i<9 or (i%4==0 and i<52) or (i%13==0 and i>52)):
                np.savetxt(output+r"\NGC2244_pos_"+str(i)+'.dat', timestepData, delimiter=',')   # X is an array
